...
export default class AdvancedBlanks extends (H5P.Question as { new(): any; }) {
    ...
    private onShowSolution = () => {
        ...
        if (this.localization.getTextFromLabel(LocalizationLabels.solutionCustomText) !== "") {
            var htmlcontentsolutiontext = document.createElement('div');
            htmlcontentsolutiontext.innerHTML = this.localization.getTextFromLabel(LocalizationLabels.solutionCustomText);
